...
class GatedGenerator(nn.Module):
    def __init__(self,
        ...
        self.coarse = nn.Sequential(
            ...
            GatedConv2d(latent_channels, latent_channels * 2, 3, 2, 1, pad_type=pad_type, activation=activation,
                        norm=norm),
            GatedConv2d(latent_channels * 2, latent_channels * 2, 3, 1, 1, pad_type=pad_type, activation=activation,
                        norm=norm),
            GatedConv2d(latent_channels * 2, latent_channels * 4, 3, 2, 1, pad_type=pad_type, activation=activation,
                        norm=norm),
            # Bottleneck
            GatedConv2d(latent_channels * 4, latent_channels * 4, 3, 1, 1, pad_type=pad_type, activation=activation,
                        norm=norm),
            GatedConv2d(latent_channels * 4, latent_channels * 4, 3, 1, 1, pad_type=pad_type, activation=activation,
                        norm=norm),
            ...
            TransposeGatedConv2d(latent_channels * 2, latent_channels, 3, 1, 1, pad_type=pad_type,
                                 activation=activation, norm=norm),
            GatedConv2d(latent_channels, latent_channels // 2, 3, 1, 1, pad_type=pad_type, activation=activation,
                        norm=norm),
        ...
        self.refine_conv = nn.Sequential(
            ...
            GatedConv2d(latent_channels, latent_channels * 2, 3, 1, 1, pad_type=pad_type, activation=activation,
                        norm=norm),
            GatedConv2d(latent_channels * 2, latent_channels * 2, 3, 2, 1, pad_type=pad_type, activation=activation,
                        norm=norm),
            GatedConv2d(latent_channels * 2, latent_channels * 4, 3, 1, 1, pad_type=pad_type, activation=activation,
                        norm=norm),
            GatedConv2d(latent_channels * 4, latent_channels * 4, 3, 1, 1, pad_type=pad_type, activation=activation,
                        norm=norm),
            GatedConv2d(latent_channels * 4, latent_channels * 4, 3, 1, 2, dilation=2, pad_type=pad_type,
        ...
        self.refine_atten_1 = nn.Sequential(
            ...
            GatedConv2d(latent_channels * 4, latent_channels * 4, 3, 1, 1, pad_type=pad_type, activation=activation,
                        norm=norm),
            GatedConv2d(latent_channels * 4, latent_channels * 4, 3, 1, 1, pad_type=pad_type, activation='relu',
                        norm=norm)
        )
        ...
        refine_combine = [
            GatedConv2d(latent_channels * 8, latent_channels * 4, 3, 1, 1, pad_type=pad_type, activation=activation,
                        norm=norm),
            GatedConv2d(latent_channels * 4, latent_channels * 4, 3, 1, 1, pad_type=pad_type, activation=activation,
                        norm=norm),
            TransposeGatedConv2d(latent_channels * 4, latent_channels * 2, 3, 1, 1, pad_type=pad_type,
        ...
        if final_tanh:
            refine_combine.append(nn.Tanh())
        self.refine_combine = nn.Sequential(*refine_combine)
    ...
    def forward(self, img, mask):
        _, _, og_h, og_w = img.shape
        img = F.interpolate(
            img,
            size=(256, int(256 * img.shape[3] / img.shape[2])),
            mode='bilinear',
            align_corners=False,
        )
        mask = F.interpolate(
    ...
    def _load(self):
        self.load_state_dict(torch.load(download_model(
            'inpaint/deepfillv2_WGAN_G_epoch40_batchsize4.pth')))
