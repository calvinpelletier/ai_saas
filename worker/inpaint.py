...
from ai_saas.ai.util.mask import union, intersection, dilate, inverse, \
    get_inner_mask, get_outer_boundary_mask
...
class Inpaint:
    ...
    def __call__(self, outer_cna, w, G, delta_g_sign, delta_g_mag):
        ...
        inner_gan_mask, gt_mask, inpaint_mask = calc_masks(
            outer_fhbc,
            new_outer_fhbc,
        )
        ...
        pre_inpaint = resize(outer_cna, INPAINT_SIZE) * gt_mask + \
            new_cna_outer * inner_gan_mask
...
def calc_masks(outer_fhbc, new_outer_fhbc):
